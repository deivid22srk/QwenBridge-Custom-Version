...
export interface QwenMessage {
  ...
  feature_config: {
    ...
    /** Present when thinking is enabled (Thinking mode); omitted for Fast. */
...
function formatPublicQwenModel(
  ...
  const baseId = String(model.id || "");
  return {
    id: noThinking ? `${baseId}-no-thinking` : baseId,
    name: noThinking
      ? `${model.name || baseId} (Fast / No Thinking)`
      : model.name || baseId,
    object: "model",
...
function expandModelVariants(model: any): PublicQwenModel[] {
  const base = formatPublicQwenModel(model, false);
  // Always-thinking models (e.g. qwen3.8-max-preview): no Fast twin
  if (isAlwaysThinkingModel(base.id)) {
    return [base];
  }
  return [base, formatPublicQwenModel(model, true)];
}
...
export async function fetchQwenModels(
  ...
  if (json.data && Array.isArray(json.data)) {
    const models = json.data.flatMap((model: any) => expandModelVariants(model));

    // Inject synthetic catalog entries when upstream omits them
    const seen = new Set(models.map((m) => m.id));
    for (const synthetic of SYNTHETIC_CATALOG_MODELS) {
      if (seen.has(synthetic.id)) continue;
      const injected = expandModelVariants({
        id: synthetic.id,
        name: synthetic.name,
        owned_by: synthetic.owned_by || "qwen",
        info: {
          created_at: Math.floor(Date.now() / 1000),
          meta: { max_context_length: synthetic.context_window },
        },
      });
...
export async function createQwenStream(
  ...
  const model = stripNoThinkingSuffix(modelId);
  const thinking = resolveThinkingConfig(modelId, enableThinking);
    ...
    messages: [
      {
        ...
        chat_type: "t2t",
        feature_config: {
          thinking_enabled: thinking.enableThinking,
          output_schema: "phase",
          research_mode: "normal",
          auto_thinking: false,
          // Max (Thinking) vs Fast — matches live Qwen UI payloads
          thinking_mode: thinking.thinkingMode,
          ...(thinking.thinkingFormat
            ? { thinking_format: thinking.thinkingFormat }
            : {}),
          auto_search: false,
        },
        extra: {
